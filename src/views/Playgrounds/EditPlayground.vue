...
export default {
  ...
  if (playground) {
    let parsedImages = [];
    if (typeof playground.images === 'string') {
      try {
        parsedImages = JSON.parse(playground.images);
      } catch (e) {
        console.error('Error parsing images JSON:', e);
      }
    } else {
      parsedImages = playground.images;
    }

    this.playground = { 
      ...playground,
      images: parsedImages.map(url => ({ name: url.split('/').pop(), url }))
    };
  }
},
...
async submitUpdate() {
  try {
    const updatedPlayground = {
      ...this.playground,
      images: JSON.stringify(this.playground.images.map(image => image.url))
    };
    await this.updatePlayground({ id: this.playground.id, data: updatedPlayground });
